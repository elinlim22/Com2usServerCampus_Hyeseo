...
public class UserController : ControllerBase
{
	...
	public UserController(UserData userData, TokenService tokenService)
	{
		_userData = userData;
	...
	{
		var user = await _userData.Users.FindAsync(request.Token); // 토큰을 받아서 데이터베이스에서 해당 토큰을 가진 유저를 찾는다.
		if (user == null)
